...
firewall_services="$(echo ${pass} |sudo firewall-cmd --list-all |grep services |cut -d ":" -f 2 |tr -d "\n" && echo)"
firewall_ports="$(echo ${pass} |sudo firewall-cmd --list-all |grep ports |cut -d ":" -f 2 |tr -d "\n" && echo)"
